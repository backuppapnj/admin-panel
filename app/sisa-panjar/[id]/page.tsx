...
export default function EditSisaPanjar() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getSisaPanjar(id);
        if (data) {
          setFormData({
            ...data,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateSisaPanjar(id, dataToSend);

      if (result.success) {
        toast({
          title: "Sukses",
          description: "Data sisa panjar berhasil diupdate!",
        });
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update detail data sisa panjar perkara.</CardDescription>
        </CardHeader>
        <CardContent>
