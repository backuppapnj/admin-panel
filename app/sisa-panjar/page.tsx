...
import { getAllSisaPanjar, deleteSisaPanjar, type SisaPanjar, type StatusSisaPanjar, NAMA_BULAN } from '@/lib/api';
import { getYearOptions, formatRupiah } from '@/lib/utils';
...
import { PlusCircle, RefreshCw, Trash2, Edit, Wallet } from 'lucide-react';
import { BlurFade } from "@/components/ui/blur-fade";

const STATUS_OPTIONS: { value: StatusSisaPanjar | 'all'; label: string }[] = [
  { value: 'all', label: 'Semua Status' },
  { value: 'belum_diambil', label: 'Belum Diambil' },
  { value: 'disetor_kas_negara', label: 'Disetor Kas Negara' },
];

export default function SisaPanjarList() {
  const [data, setData] = useState<SisaPanjar[]>([]);
  const [loading, setLoading] = useState(true);
  const [filterTahun, setFilterTahun] = useState<string>("all");
  const [filterStatus, setFilterStatus] = useState<StatusSisaPanjar | 'all'>("all");
  ...
  const loadData = async (page = 1) => {
    ...
    try {
      ...
      const result = await getAllSisaPanjar(year, status, undefined, page);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSisaPanjar(deleteId);
      toast({
        title: "Sukses",
  ...
  const getStatusBadge = (status: StatusSisaPanjar) => {
    if (status === 'belum_diambil') {
      return <Badge variant="secondary" className="bg-amber-100 text-amber-800 hover:bg-amber-100">Belum Diambil</Badge>;
    }
    return <Badge variant="secondary" className="bg-green-100 text-green-800 hover:bg-green-100">Disetor Kas Negara</Badge>;
  };
    ...
    <div className="space-y-6">
      <BlurFade delay={0.1} inView>
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            <h2 className="text-3xl font-bold tracking-tight">Sisa Panjar Perkara</h2>
            <p className="text-muted-foreground">Kelola data sisa panjar yang belum diambil dan yang sudah disetor ke Kas Negara.</p>
          </div>
          <Link href="/sisa-panjar/tambah">
      ...
      <BlurFade delay={0.2} inView>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
            <CardTitle className="text-lg font-medium">Data Sisa Panjar</CardTitle>
            <div className="flex items-center gap-2">
              ...
              <Select value={filterStatus} onValueChange={(v) => setFilterStatus(v as StatusSisaPanjar | 'all')}>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Nama Penggugat/Pemohon</TableHead>
                  ...
                  {loading ? (
                    ...
                    data.map((item, index) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell>{item.nama_penggugat_pemohon}</TableCell>
                        <TableCell className="text-right font-mono">{formatRupiah(item.jumlah_sisa_panjar)}</TableCell>
                        <TableCell>{NAMA_BULAN[item.bulan]}</TableCell>
                        <TableCell>{getStatusBadge(item.status)}</TableCell>
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Link href={`/sisa-panjar/${item.id}/edit`}>
      ...
      <MagicDeleteDialog
        ...
        description="Apakah Anda yakin ingin menghapus data sisa panjar ini? Tindakan ini tidak dapat dibatalkan."
      />
    </div>
  );
}
